...
export default function App() {
  ...
  return <main className="app-shell">
    ...
    {tab === 'map' && <>
      ...
      <MapSheet selected={selected} saved={selected ? saved.includes(selected.id) : false} route={route} routing={routing} routeMessage={routeMessage} onClose={() => setSelected(null)} onSave={() => selected && toggleSaved(selected.id)} onNavigate={() => selected && navigate(selected)} />
      {route && !selected && <RouteControl route={route} onFinish={() => { setRoute(null); setRouteMessage('') }} />}
    </>}
...
function RouteControl({ route, onFinish }: { route: RouteInfo; onFinish: () => void }) {
  const minutes = Math.max(1, Math.round(route.duration / 60))
  return <section className="route-control glass"><div><p className="eyebrow">ПЕШИЙ МАРШРУТ</p><b>{Math.round(route.distance)} м · {minutes} мин</b></div><button onClick={onFinish}>Завершить</button></section>
}
